...
describe('Mechanarium assembly studio', () => {
  ...
  it('builds beams, custom ports, and ropes while paused', async () => {
    ...
    expect(screen.getByText('Mounted to Beam')).toBeInTheDocument()
    expect(screen.getByText('Follows position and rotation')).toBeInTheDocument()
    expect(screen.getByRole('button', { name: 'Select Beam' })).toBeEnabled()
  ...
  it('previews and explicitly confirms structural snaps', async () => {
    ...
    expect(screen.getByText('First port armed')).toBeInTheDocument()
  ...
  it('builds a measurement-only lab and records a reviewable 120 Hz trial', async () => {
    ...
    const cancelFrame = vi.spyOn(window, 'cancelAnimationFrame').mockImplementation(() => {})
    const user = userEvent.setup(); render(<App />)
    await user.click(screen.getByRole('button', { name: 'Add Ruler' }))
    await user.click(screen.getByRole('button', { name: 'Add Photogate' }))
    await user.click(screen.getByRole('button', { name: 'Add Photogate' }))
    await user.click(screen.getByRole('tab', { name: 'Lab' }))
    expect(screen.getByText('Motion timing laboratory')).toBeInTheDocument()
    expect(screen.getByText(/Distance 4.000 m/)).toBeInTheDocument()
    expect(screen.queryByRole('dialog', { name: 'Snap placement' })).not.toBeInTheDocument()
    await user.type(screen.getByLabelText('Prediction / notes'), 'A steeper incline should increase acceleration.')
    await user.click(screen.getByRole('button', { name: 'Record trial' }))
    expect(screen.getByText(/Armed — press Run/)).toBeInTheDocument()
    await user.click(screen.getByRole('button', { name: 'Run' }))
    act(() => callbacks.shift()(10)); act(() => callbacks.shift()(50))
    await user.click(screen.getByRole('button', { name: 'Pause' }))
