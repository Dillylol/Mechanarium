...
export const ACTION_TYPES = Object.freeze([
  'add_body', 'add_track', 'add_beam', 'add_wheel', 'add_connector', 'add_port', 'add_instrument',
...
export const ACTION_TARGETS = Object.freeze([
  'sphere', 'box', 'wheel', 'ramp', 'floor', 'spring', 'rope', 'beam', 'attachment',
...
const targetsByAction = new Map([
  ...
  ['load_preset', new Set(ACTION_TARGETS.filter((target) => target.endsWith('-motion') || target.endsWith('-collision') || target.endsWith('-incline') || target.endsWith('-oscillator') || target.endsWith('-pendulum') || target.endsWith('-atwood')))],
  ['none', new Set([null])],
])
...
export function validateAgentInput(input) {
  ...
  if (!isRecord(input.scenario) || !Array.isArray(input.scenario.bodies) || !Array.isArray(input.scenario.tracks)) throw new TypeError('A valid Scenario v3 summary is required.')
